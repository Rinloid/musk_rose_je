...
varying vec2 uv;
varying vec3 sunPos, shadowLightPos;
...
#define SKY_COL  vec3(0.65, 0.8, 1.0)
...
vec3 sky = getSky(skyPos, shadowLightPos, -sunPos, SKY_COL, daylight, rainStrength, frameTimeCounter, moonPhase);
...
#endif
{
    ...
    amnientLightFactor = 0.8;
    dirLightFactor = mix(0.0, diffuse, shadows.a);
    emissiveLightFactor = uv1.x * uv1.x * uv1.x * uv1.x * uv1.x;
    ambientLightCol = mix(mix(vec3(0.0), TORCHLIGHT_COL, emissiveLightFactor), daylightCol, dirLightFactor);
    ...
    */
    ...
    albedo = mix(albedo, fogCol, clamp(fogFactor * 2.0, 0.0, 1.0));
}
...
uniform mat4 gbufferModelView, gbufferModelViewInverse;
uniform vec3 sunPosition, shadowLightPosition;

...
if (texture(depthtex0, uv).r != 1.0) { // Is not sky
...
vec3 underwaterfogCol = (waterAbsorb + waterScatter) * 0.75 * clamp(sin(sunPos.y) * (1.0 - rainStrength), 0.25, 1.0);
	...
	sunRayCol = 1.0 - exp(-1.0 * sunRayCol);

	int raySteps = 32;
	vec4 rayOrig = getRelPos(depthtex0, uv);
	vec4 rayCol = vec4(0.0, 0.0, 0.0, 0.0);
	float rayFactor = 0.0;
	...
	for (int i = 0; i < raySteps; i++) {
		...
		if (texture(
				...
				shadowtex0
			#endif
			, rayPos.xy).r > rayPos.z) {
			if (texture(depthtex0, uv).r != 1.0) rayFactor = mix(rayPos.w, rayFactor, exp2(length(rayOrig.xyz) * -0.0625));
			#ifdef ENABLE_COLED_RAYS
				if (texture(shadowtex0, rayPos.xy).r < rayPos.z) {
					if (texture(depthtex0, uv).r == 1.0) rayFactor = mix(rayPos.w, rayFactor, exp2(length(rayOrig.xyz) * -0.0625));
					rayCol = mix(texture(shadowcolor0, rayPos.xy), rayCol, exp2(length(rayOrig.xyz) * -0.0625));
				}
			#endif
		}
	}

	albedo = mix(albedo, mix(sunRayCol, rayCol.rgb * 2.0, rayCol.a), clamp(sunRayCol * rayFactor, 0.0, 1.0));
	if (translucent.a > 0.0) {
		translucent.rgb = mix(translucent.rgb, mix(sunRayCol, rayCol.rgb, rayCol.a), clamp(sunRayCol * rayFactor, 0.0, 1.0));
	}
#endif

...
const float shadowDistortFactor = 0.03;
...
float getDistortFactor(const vec2 pos) {
	return pow(abs(pos.x * pos.x * pos.x) + abs(pos.y * pos.y * pos.y), 1.0 / 3.0) + shadowDistortFactor;
}
vec3 distort(const vec3 pos, const float factor) {
	return vec3(pos.xy / factor, pos.z * 0.2);
}
vec3 distort(const vec3 pos) {
	return distort(pos, pow(abs(pos.x * pos.x * pos.x) + abs(pos.y * pos.y * pos.y), 1.0 / 3.0) + shadowDistortFactor);
}
